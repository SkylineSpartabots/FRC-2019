...
public class PathExecuter extends Command   {    
    ...
    public PathExecuter(Waypoint[] points, String FileName)   {
        requires(Robot.driveTrain);
        try {
            Trajectory.Config config = new Trajectory.Config(Trajectory.FitMethod.HERMITE_CUBIC, Trajectory.Config.SAMPLES_HIGH, 0.05, RobotMap.MaxVelocity, 2.0, 60.0);
    ...
    public void updateMotorOutputs(double LeftEncoderDistance, double RightEncoderDistance) {
        ...
        PathingLog.writeNewData("Pathing Update: LeftMotorOutput: " + LeftMotorOutput + " RightMotorOutput: " + RightMotorOutput + " DesiredHeading: " + desired_heading + " Actual Heading: " + Robot.rps.getAngle());
    }
